...
const SendedMail = ({ token }) => {
    ...
    useEffect(() => {
        const fetchMail = async () => {
            try {
                const response = await axios.post('http://127.0.0.1:8000/sendedmail/', {
                    token,
                });
    ...
    return (
        <div className={'sended-mail-container'}>
            {error ? (
                <div className={'not-mail'}>{error}</div>
            ) : (
                Array.isArray(mail) &&
